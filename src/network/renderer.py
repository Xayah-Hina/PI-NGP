...
class NeRFRenderer(torch.nn.Module):
    def __init__(self,
        ...
        if cuda_ray:
            # density grid
            self.register_buffer('density_grid', torch.zeros([cascade, grid_size ** 3]))  # [CAS, H * H * H]
            self.register_buffer('density_bitfield', torch.zeros(cascade * grid_size ** 3 // 8, dtype=torch.uint8))  # [CAS * H * H * H // 8]
            self.mean_density = 0
            self.iter_density = 0
            # step counter
    ...
    def update_extra_state(self, decay=0.95, S=128):
        ...
        # TODO: why no need of maxpool ?
            ...
            N = self.runtime_params['grid_size'] ** 3 // 4  # H * H * H / 4
            for cas in range(self.runtime_params['cascade']):
    ...
    def run_cuda(self, rays_o, rays_d, dt_gamma, bg_color, perturb, force_all_rays, max_steps, T_thresh):
        ...
        if self.training:
            ...
            xyzs, dirs, deltas, rays = raymarching.march_rays_train(rays_o, rays_d, self.runtime_params['bound'], self.density_bitfield, self.runtime_params['cascade'], self.runtime_params['grid_size'], nears, fars, counter, self.mean_count, perturb, 128, force_all_rays, dt_gamma, max_steps)
            ...
            if len(sigmas.shape) == 2:
                raise NotImplementedError('not implemented for CCNeRF')
            else:
            ...
            while step < max_steps:
                ...
                xyzs, dirs, deltas = raymarching.march_rays(n_alive, n_step, rays_alive, rays_t, rays_o, rays_d, self.runtime_params['bound'], self.density_bitfield, self.runtime_params['cascade'], self.runtime_params['grid_size'], nears, fars, 128, perturb if step == 0 else False, dt_gamma, max_steps)
